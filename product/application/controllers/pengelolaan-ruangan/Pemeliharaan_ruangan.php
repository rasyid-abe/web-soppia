...
class Pemeliharaan_ruangan extends CI_Controller {
 	protected  $name = 'Pemeliharaan ruangan'; 
    protected  $model = 'umum/pengelolaan-ruangan/Rpemeliharaan_model';
 	protected  $breadcrumb1 = 'Dashboard'; 
 	protected  $breadcrumb2 = 'Pengelolaan Ruangan'; 
 	protected  $breadcrumb3 = 'Pemeliharaan Ruangan'; 
	...
	public function index(){
		permissions('melihat-data-pemeliharaan-ruangan');
		$urlnow = uri_string();
		$data = array(
	...
	public function getdata(){
        ...
        foreach ($list as $field) {
            ...
            $row[] = $field->petugas;
            $row[] = $this->actiontable($field->id_pemeliharaan);
            $data[] = $row;
        }
	...
	public function actiontable($id){
		if(accessperm('melihat-data-pemeliharaan-ruangan')){
		...
		if(accessperm('merubah-data-pemeliharaan-ruangan')){
		...
		if(accessperm('menghapus-data-pemeliharaan-ruangan')){
	...
	public function add(){
		permissions('menambahkan-data-pemeliharaan-ruangan');
	...
	public function store(){	
		...
		$ket = $this->security->xss_clean(html_escape($this->input->post("alasan_perbaikan")));
		$datatitik = $this->security->xss_clean(html_escape($this->input->post("biaya")));
		
		$biaya = str_replace(".", "", $datatitik);
		if($biaya == ""){
			$biaya = 0;
		}else{
			$biaya = $biaya;
		...
		$this->form_validation->set_rules('alasan_perbaikan','Keterangan','trim|xss_clean');	
	...
	public function edit($id){
		permissions('merubah-data-pemeliharaan-ruangan');
		$urlnow = $this->uri->segment(1).'/'.$this->uri->segment(2);
		$data = array(
			'breadcrumb1'=>$this->breadcrumb1,
			'breadcrumb2'=>$this->breadcrumb2,
	...
	public function update($id){	
		...
		$pet = $this->security->xss_clean(html_escape($this->input->post("petugas")));
		$ket = $this->security->xss_clean(html_escape($this->input->post("alasan_perbaikan")));
		$nilaiadatitik = $this->security->xss_clean(html_escape($this->input->post("biaya")));
		
		$biaya = str_replace(str_split(",."),"", $nilaiadatitik);
	...
	public function delete($id){
		permissions('menghapus-data-pemeliharaan-ruangan');
