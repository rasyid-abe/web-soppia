...
class Materi_sub_bab extends CI_Controller {
 	protected  $name = 'Materi sub bab'; 
    protected  $model = 'data/materi/Rmaterisubbab_model'; 
	...
	public function index(){
		permissions('melihat-data-materi-sub-bab');
		$urlnow = uri_string();
		$data = array(
	...
	public function getdata(){
        ...
        foreach ($list as $field) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = $field->Desc_SubBab;
            $row[] = $field->Desc_Materi_n_Aktifitas;
            $row[] = $this->actiontable($field->Kd_SubBab);
	...
	public function actiontable($id){
		if(accessperm('melihat-data-materi-sub-bab')){
		...
		if(accessperm('merubah-data-materi-sub-bab')){
		...
		if(accessperm('menghapus-data-materi-sub-bab')){
	...
	public function add(){
		permissions('menambahkan-data-materi-sub-bab');
		...
		$data = array(
			...
			'urlback'=>$this->db->where("url",$urlnow)->get("menu"),
			'getmatandact' => $this->db->get("mst_materi_n_aktifitas")
		);
	...
	public function store(){	
		...
		$matandact = $this->security->xss_clean(html_escape($this->input->post("matandact")));

		$this->form_validation->set_rules('name', 'Name Description', 'trim|required|is_unique[mst_materi_subbab.Desc_SubBab]|xss_clean');
		$this->form_validation->set_rules('matandact', 'Materi Dan Aktifitas', 'trim|required|xss_clean');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				'Desc_SubBab'=>$name,
				'FKd_Materi_n_Aktifitas'=>$matandact,
			);
	...
	public function edit($id){
		permissions('merubah-data-materi-sub-bab');
		$urlnow = $this->uri->segment(1).'/'.$this->uri->segment(2);
		$data = array(
			'breadcrumb1'=>$this->breadcrumb1,
			'breadcrumb2'=>$this->breadcrumb2,
	...
	public function delete($id){
		permissions('menghapus-data-materi-sub-bab');
