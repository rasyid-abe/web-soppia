...
class Perpus_buku extends CI_Controller {
 	protected  $name = 'Data Buku'; 
    protected  $model = 'umum/perpustakaan/Rperpusbuku_model';
 	protected  $breadcrumb1 = 'Dashboard'; 
 	protected  $breadcrumb2 = 'Perpustakaan'; 
 	protected  $breadcrumb3 = 'Data Buku'; 
	...
	public function getdata(){
        ...
        foreach ($list as $field) {
            ...
            $row[] = $field->pengarang;
            $row[] = $field->jumlah_buku." Buku";
            $row[] = $field->tahun_buku;
            $row[] = $field->rak;
            $row[] = $this->actiontable($field->id_buku);
	...
	public function store(){	
		...
		$buku = $this->security->xss_clean(html_escape($this->input->post("judul_buku")));
		$penerbit = $this->security->xss_clean(html_escape($this->input->post("penerbit")));
		$pengarang = $this->security->xss_clean(html_escape($this->input->post("pengarang")));
		$tahun = $this->security->xss_clean(html_escape($this->input->post("tahun_buku")));
		$jml = $this->security->xss_clean(html_escape($this->input->post("jumlah_buku")));
		$rak = $this->security->xss_clean(html_escape($this->input->post("rak")));
		
		$this->form_validation->set_rules('judul_buku', 'Judul Buku', 'trim|required|xss_clean');
		$this->form_validation->set_rules('penerbit', 'Penerbit', 'trim|xss_clean');
		$this->form_validation->set_rules('pengarang', 'Pengarang', 'trim|xss_clean');
		$this->form_validation->set_rules('tahun_buku', 'Tahun', 'trim|xss_clean');
		$this->form_validation->set_rules('jumlah_buku', 'Jumlah Buku', 'trim|xss_clean');
		$this->form_validation->set_rules('rak', 'Lokasi Rak', 'trim|xss_clean');
