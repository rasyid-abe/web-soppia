...
class Pembukaan_kunci_entry extends CI_Controller {
 	protected  $name = 'Pembukaan kunci entry'; 
    protected  $model = 'pelatihan/peserta/Rpembukaankunci_model'; 

 	protected  $breadcrumb1 = 'Dashboard'; 
 	protected  $breadcrumb2 = 'Peserta'; 
 	protected  $breadcrumb3 = 'Pembukaan Kunci Entry'; 
	...
	public function status($id){
	    ...
	    if($cek->num_rows()>0){
	        if( ($cek->row()->Alasan_Pembukaan_Kuncian_Pejabat1 != null && $cek->row()->Alasan_Pembukaan_Kuncian_Pejabat2 != null) ){
	            return "<span class='label label-success'>Data telah dibuka</span>";
	        }else if($cek->row()->Alasan_Pembukaan_Kuncian_Pejabat1 != null && $cek->row()->Alasan_Pembukaan_Kuncian_Pejabat2 == null ){
	            return "<span class='label label-warning'>Menunggu persetujuan pejabat kedua</span>";
	        }else if($cek->row()->Alasan_Pembukaan_Kuncian_Pejabat1 == null && $cek->row()->Alasan_Pembukaan_Kuncian_Pejabat2 != null ){
	            return "<span class='label label-warning'>Menunggu persetujuan pejabat pertama</span>";
	        }else{
	            return "<span class='label label-danger'>Menunggu Persetujuan 2 Pejabat</span>";
	        }
	    }else{
	...
	public function actiontable($id){
        ...
        if(accessperm('membuka-kuncian-data-pembukaan-kunci-entry')){
			$btn .= "<a href='".base_url('peserta/'.url_title(strtolower($this->name))."/buka-kuncian/".$id)."' class='btn btn-xs btn-success' data-toggle='tooltip' title='Buka Kuncian Entry'> <i class='fa fa-key'></i> Buka Kunci </a> ";
        }
        if(accessperm('menghapus-data-pembukaan-kunci-entry')){
	...
	public function verifikasi($id){
		...
		recordlog("Verifikasi Kunci Data Peserta");
		$this->load->view("pages/modul/pelatihan/peserta/".url_title(strtolower($this->name))."/verifikasi",$data);
	}
	...
	public function verifikasi_proses($id){
		...
		if($username != "anonim" && $password != "anonim"){
			...
			if($cek->status == false){
				$rsl = array(
					'status'=>'error',
					'msg'=> 'Anda tidak berhak membuka kuncian pendaftaran calon peserta'
				);
			}else{
				if($cek->is_active == 1){						
					$rsl = array(
						'status'=>'sukses',
						'msg'=> base_url('peserta/'.url_title(strtolower($this->name)).'/buka-kuncian/'.$id)
					);
				}else{				
					$rsl = array(
						'status'=>'error',
	...
	public function save_buka_kuncian($id){
		...
		if ($this->form_validation->run() == FALSE) {
			...
			redirect(base_url('peserta/'.url_title(strtolower($this->name)).'/buka-kuncian/'.$id));
		}else{	
			...
			if( $cekpst->row()->Alasan_Pembukaan_Kuncian_Pejabat1 != null && $cekpst->row()->Alasan_Pembukaan_Kuncian_Pejabat2 != null ){
    			...
    			$this->session->set_flashdata('success', 'Data berhasil di simpan dan sudah di buka dari kuncian');
