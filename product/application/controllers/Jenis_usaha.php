...
class Jenis_usaha extends CI_Controller { 
    protected  $name = 'Jenis usaha'; 
    protected  $model = 'referensi/Rjenis_usaha_model'; 
	...
	public function index(){
		permissions('melihat-data-jenis-usaha');
		$data = array(
	...
	public function getdata(){
        ...
        foreach ($list as $field) {
            ...
            $row[] = $this->actiontable($field->Kd_JenisUsaha);
	...
	public function actiontable($id){
		if(accessperm('melihat-data-jenis-usaha')){
		...
		if(accessperm('merubah-data-jenis-usaha')){
		...
		if(accessperm('menghapus-data-jenis-usaha')){
	...
	public function add(){
		permissions('menambahkan-data-jenis-usaha');
		$data = array(
			'breadcrumb1'=>'Dashboard',
			'breadcrumb2'=>$this->name,
			'titlepage'=>$this->name,
	...
	public function store(){	
		...
		$this->form_validation->set_rules('name', 'Name', 'trim|required|is_unique[ref_jenisusaha.Desc_JenisUsaha]|xss_clean');
		if ($this->form_validation->run() == FALSE) {
			$this->session->set_flashdata(array( 'error'=>validation_errors(),'oldinput'=> $this->security->xss_clean(html_escape($this->input->post())) ));
			redirect(base_url(url_title(strtolower($this->name)).'/add'));
		}else{
			$data = array(
				'Desc_JenisUsaha'=>$name
			);
	...
	public function edit($id){
		permissions('merubah-data-jenis-usaha');
		$data = array(
			'breadcrumb1'=>'Dashboard',
			'breadcrumb2'=>$this->name,
			'titlepage'=>$this->name,
	...
	public function delete($id){
		permissions('menghapus-data-jenis-usaha');
