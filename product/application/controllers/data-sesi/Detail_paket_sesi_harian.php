...
class Detail_paket_sesi_harian extends CI_Controller {
 	protected  $name = 'Detail paket sesi harian'; 
    protected  $model = 'referensi/data-sesi/Rdetailpaketsesiharian_model'; 
	...
	public function index(){
		permissions('melihat-data-detail-paket-sesi-harian');
		$urlnow = uri_string();
		$data = array(
			'breadcrumb1'=>$this->breadcrumb1,
			'breadcrumb2'=>$this->breadcrumb2,
			'breadcrumb3'=>'Detail Paket Sesi Harian',
			'titlepage'=>'Detail Paket Sesi Harian',
			'subtitlepage'=>'Data Detail Paket Sesi Harian',
			'titlebox'=>'Manage Detail Paket Sesi Harian',
	...
	public function getdata(){
        ...
        foreach ($list as $field) {
            ...
            $row[] = $field->Desc_Sesi.' - '.$field->Keterangan;
            $row[] = $this->actiontable($field->Kd_detail);
	...
	public function actiontable($id){
		if(accessperm('melihat-data-detail-paket-sesi-harian')){
			$btn = "<a href='".base_url('data-sesi/'.url_title(strtolower($this->name))."/view/".$id)."' class='btn btn-box-tool view-data' data-toggle='tooltip' title='View Data'> <i class='fa fa-search-plus'></i> View </a>";
        }else{
			$btn = "<a class='btn btn-box-tool' data-toggle='tooltip' title='No Access'> <i class='fa fa-search-plus'></i> No Access </a>";
        }
		if(accessperm('merubah-data-detail-paket-sesi-harian')){
			$btn .= "<a href='".base_url('data-sesi/'.url_title(strtolower($this->name))."/edit/".$id)."' class='btn btn-box-tool' data-toggle='tooltip'
		...
		if(accessperm('menghapus-data-detail-paket-sesi-harian')){
			$btn .= "<a href='".base_url('data-sesi/'.url_title(strtolower($this->name))."/delete/".$id)."' class='btn btn-box-tool delete-data' data-toggle='tooltip' title='Delete Data'> <i class='fa fa-trash'></i> Delete </a>";	
        }else{
	...
	public function add(){
		permissions('menambahkan-data-detail-paket-sesi-harian');
		...
		$data = array(
			...
			'titlepage'=>'Detail Paket Sesi Harian',
			'subtitlepage'=>'Data Detail Paket Sesi Harian',
			'titlebox'=>'Add Detail Paket Sesi Harian',
			'fkd_paket_sesi_harian'=>$this->db->get("ref_paket_sesi_harian"),
	...
	public function store(){	
		...
		$fkd1 = $this->security->xss_clean(html_escape($this->input->post("FKd_Paket_Sesi_Harian")));
		$fkd2 = $this->security->xss_clean(html_escape($this->input->post("FKd_Sesi_Satuan")));
		
		$this->form_validation->set_rules('FKd_Paket_Sesi_Harian', 'FKd_Paket_Sesi_Harian', 'trim|required|xss_clean');
		$this->form_validation->set_rules('FKd_Sesi_Satuan', 'FKd_Sesi_Satuan', 'trim|required|xss_clean');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$data = array(
				'FKd_Paket_Sesi_Harian'=>$fkd1,
				'FKd_Sesi_Satuan'=>$fkd2
			);
	...
	public function edit($id){
		permissions('merubah-data-detail-paket-sesi-harian');
		$urlnow = $this->uri->segment(1).'/'.$this->uri->segment(2);
		$data = array(
			'breadcrumb1'=>$this->breadcrumb1,
			'breadcrumb2'=>$this->breadcrumb2,
			...
			'titlepage'=>'Detail Paket Sesi Harian',
			'subtitlepage'=>'Data Detail Paket Sesi Harian',
			'titlebox'=>'Edit Detail Paket Sesi Harian',
	...
	public function delete($id){
		permissions('menghapus-data-detail-paket-sesi-harian');
