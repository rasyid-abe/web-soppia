...
<section class="content">
  ...
  <div class="box box-warning">
    ...
    <div class="box-body">
      <form action="<?=base_url($this->uri->segment(1)."/update/".$perm->idpermission)?>" method="POST">
        <input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash()?>" />        
        <div class="form-group col-sm-12 <?=(!empty(form_error('name')))? 'has-error' :'';?>">
          <div class="col-sm-12">
            Nama Permission/Izin <br/>
            <input type="text" class="form-control" name="name" id="name" placeholder="Name"  value="<?=$perm->name?>" >
          </div>
        </div>
        ...
        <div class="form-group col-sm-12">
          <div class="col-sm-12">
            ...
            <select name="menu" class="form-control select2" style="width: 100%;">
              <option value="" readonly disabled selected>UNTUK MENU</option>
              <?php
              foreach ($menu->result() as $mn) {
                //if($mn->parent != '' || $mn->parent != null){             
                  $slc = ($perm->idmenu==$mn->idmenu)? 'selected':'';  
                    if($mn->active =='1'){                
              ?>
              <option value="<?=$mn->idmenu?>" <?=$slc?> ><?=$mn->name?></option>
              <?php                  
                    }
                //}
              }
        ...
        <div class="form-group col-sm-12 <?=(!empty(form_error('description')))? 'has-error' :'';?>">
          <div class="col-sm-12">
            Keterangan <br/>
            <textarea name="description" class="form-control" placeholder="Description"><?=$perm->description?></textarea>
          </div>
        </div>
