  ...
  <div class="box box-primary">
    ...
    <div class="box-body">
        <form action="<?=base_url($this->uri->segment(1).'/'.$this->uri->segment(2)."/store")?>" method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" name="idkelasqia" value="<?=$idkelasqia?>" />   
...
<script type="text/javascript">  
  $(function () {
    ...
    $('table.dt-table').DataTable({
      ...
      "serverSide": true, 
      "order": [], 
      <?php if(accessperm('mengekspor-data-kelas-qia')){ ?>
      "dom": 'Bfrtip',
      "buttons": [
      ...
      <?php } ?>
      "ajax": {
        "url": "<?php echo site_url($this->uri->segment(1).'/'.$this->uri->segment(2).'/getdataqia/'.$idkelasqia)?>",
        "type": "POST",
...
<script type="text/javascript">  
  $(function () {
    $("#idpeserta").select2({
        ...
        ajax: {
            dataType: 'json',
            url: '<?=base_url('pendaftaran-kelas/kelas-qia/getpeserta')?>',
            delay: 250,
