...
<section class="content">
  ...
  <div class="box" style="border-top:0px solid">
    ...
    <div class="box-body">
      <form action="<?=base_url($this->uri->segment(1).'/'.$this->uri->segment(2)."/update/".$dtdefault->Id_AddKontrak)?>" method="POST" enctype="multipart/form-data">
        <input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash()?>" />    

        <div class="form-group col-sm-12">
          <div class="col-sm-6" style="color:#00008B">
            Addendum Kontrak<br/>
            <input type="text" class="form-control" name="Desc_AddKontrak" id="Desc_AddKontrak" placeholder="Desc Addendum Kontrak" value="<?=$dtdefault->Desc_AddKontrak?>" >
          </div>
        </div>
        ...
        <div  id="temp-pershinstansi">
            ...
            <div class="col-sm-6" style="color:#00008B">
              ...
              <select name="FId_PershInstansi" class="form-control select2" style="width: 100%;">
                ...
                foreach ($fid_pershinstansi->result() as $fid) {
                  $slca = ($dtdefault->FId_PershInstansi==$fid->Id_PershInstansi)? 'selected':'readonly disabled';
                ?>
                <option value="<?=$fid->Id_PershInstansi?>" <?=$slca?> ><?=$fid->Desc_PershInstansi?></option>
                <?php
                }
                ?>
              </select>
          ...
          <div class="form-group col-sm-12">
            <div class="col-sm-6" style="color:#00008B">
              File Sebelumnya : <a href="<?=base_url('uploads/fileapps/'.$dtdefault->File_Lampiran)?>"><i class="fa fa-download"></i> File Lampiran</a>
