                ...
                <div class="panel-heading">
                  <h4 class="panel-title">
                    <a data-toggle="collapse" data-parent="#accordion" href="#collapse<?=$noomas?>"><?=$mn->name?></a>
                      <label class="pull-right"  style='<?=(checkmenurole($role->idrole,$mn->idmenu))? 'background:green !important;border-radius:10px':'';?>' >
                          <?php $check = (checkmenurole($role->idrole,$mn->idmenu))? 'checked':''; ?>
                        <input type="checkbox" class="flat-red set-menu-checkbox" data-role="<?=$role->idrole?>" data-perm="<?=$mn->idmenu?>"  <?=$check?> disabled >
                      </label>
                  </h4>
                </div>
                        ...
                        <div class="panel-heading">
                          <h4 class="panel-title">
                            <a data-toggle="collapse" data-parent="#accordion<?=$noomas?>" href="#collapseaa<?=$noomas?><?=$noomasa?>"><?=$cekank->name?></a>
                              <label class="pull-right"   style='<?=(checkmenurole($role->idrole,$cekank->idmenu))? 'background:green !important;border-radius:10px':'';?>' >
                                  <?php $check = (checkmenurole($role->idrole,$cekank->idmenu))? 'checked':''; ?>
                                <input type="checkbox" class="flat-red set-menu-checkbox" data-role="<?=$role->idrole?>" data-perm="<?=$cekank->idmenu?>"   <?=$check?> disabled >
                              </label>
                          </h4>
                        </div>
                                  ...
                                  <div class="panel-heading">
                                    <h4 class="panel-title">
                                      <a data-toggle="collapse" data-parent="#accordion<?=$noomasa?>" href="#collapseaa<?=$noomas?><?=$noomasa?><?=$nomasaabb?>"><?=$kv->name?></a>
                                        ...
                                        <label class="pull-right"  style='<?=(checkmenurole($role->idrole,$kv->idmenu))? 'background:green !important;border-radius:10px':'';?>' >
                                          <?php $check = (checkmenurole($role->idrole,$kv->idmenu))? 'checked':''; ?>
                                          <input type="checkbox" class="flat-red set-menu-checkbox" data-role="<?=$role->idrole?>" data-perm="<?=$kv->idmenu?>"  <?=$check?> disabled >
                                        </label>
                                    </h4>
                                  </div>
                                      ...
                                      foreach ($getperm->result() as $val) {
                                        ...
                                        <div class="panel-body">
                                          <label>
                                            ...
                                            <input type="checkbox" class="flat-red set-role-checkbox" data-role="<?=$role->idrole?>" data-perm="<?=$val->idpermission?>" <?=$check?>>
                                            <?=ucfirst($val->name)?>
                                          </label>
                                          <br/>
                                            <?=$val->description?>
...
<script type="text/javascript">  
  $(function(){    
    ...
    $(document).on("ifChanged","input[type='checkbox'].set-menu-checkbox",function(){
      var thisval = $(this);
      //alert(thisval.val());
      var Thislink = _BASE_URL_+'role/permmenurole/'+thisval.attr("data-role")+'/'+thisval.attr("data-perm");
