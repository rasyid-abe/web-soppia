  ...
  <div class="box box-primary">
    <div class="box-header with-border">
      ...
      <div class="box-tools pull-right">
        <?php if(accessperm('menambahkan-data-menu')){ ?>
          <a href="<?=base_url($this->uri->segment(1)."/add")?>" class="btn btn-box-tool" data-toggle="tooltip" title="Add Data"> <i class="fa fa-plus-circle"></i> Add Data</a>
        <?php }else{?>
          <a class="btn btn-box-tool" data-toggle="tooltip" title="No Access"><i class="fa fa-plus-circle"></i> No Access</a>
        <?php }?>
        <!-- <?php if(accessperm('mengimport-data-menu')){ ?>        
          <a href="<?=base_url($this->uri->segment(1)."/import")?>" class="btn btn-box-tool" data-toggle="tooltip" title="Import Data"> <i class="fa fa-file-text"></i> Import Data</a>
        <?php }else{?>
          <a class="btn btn-box-tool" data-toggle="tooltip" title="No Access"><i class="fa fa-file-text"></i> No Access</a>
        <?php }?> -->
        <a href="<?=base_url($this->uri->segment(1)."/set-position-menu")?>" class="btn btn-box-tool" data-toggle="tooltip" title="Mengatur Position Menu"><i class="fa fa-th"></i> Set Position Menu</a>
...
<script type="text/javascript">  
  $(function () {
    ...
    $('table.dt-table').DataTable({
      ...
      "serverSide": true, 
      "order": [], 
      <?php if(accessperm('mengekspor-data-menu')){ ?>
      "dom": 'Bfrtip',
      "buttons": [
