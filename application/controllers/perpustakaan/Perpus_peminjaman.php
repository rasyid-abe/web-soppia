...
class Perpus_peminjaman extends CI_Controller {
 	protected  $name = 'Data peminjaman'; 
    protected  $model = 'umum/perpustakaan/Rperpuspinjam_model';
 	protected  $breadcrumb1 = 'Dashboard'; 
 	protected  $breadcrumb2 = 'Perpustakaan'; 
 	protected  $breadcrumb3 = 'Data Peminjaman'; 
	...
	public function getdata(){
        ...
        foreach ($list as $field) {
            ...
            $row[] = $this->actiontable($field->id_pinjam);
	...
	public function store(){	
		...
		$anggota = $this->security->xss_clean(html_escape($this->input->post("id_anggota")));
		$buku = $this->security->xss_clean(html_escape($this->input->post("id_buku")));
		$jml = $this->security->xss_clean(html_escape($this->input->post("jml_pinjam")));
		$tglp = $this->security->xss_clean(html_escape($this->input->post("tgl_pinjam")));
		$tglk = $this->security->xss_clean(html_escape($this->input->post("tgl_kembali")));
		
		$this->form_validation->set_rules('id_anggota', 'Nama Anggota', 'trim|required|xss_clean');
		$this->form_validation->set_rules('id_buku', 'Judul Buku', 'trim|xss_clean');
		$this->form_validation->set_rules('jml_pinjam', 'Jumlah Buku', 'trim|xss_clean');
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$stokbuku = $this->db->query("select jumlah_buku from perpus_buku where id_buku='$buku'")->row_array();
			$jumlah = $stokbuku['jumlah_buku'] - $jml;
			if($stokbuku['jumlah_buku'] < $jml){
			    $this->session->set_flashdata('error', 'Stok Buku Tidak Cukup');
			    redirect(base_url('perpustakaan/'.url_title(strtolower($this->name)).'/add')); 
			} else {
			    
			    $stokupdate = $this->db->query("update perpus_buku set jumlah_buku='$jumlah' where id_buku='$buku'");
