...
<?php
$cekaddendumlebihbesar = $this->db->where(array( "FId_ProformaKontrak"=>$this->uri->segment(3) ))->get("mst_addendumkontrak");
$counting = 0;
if($cekaddendumlebihbesar->num_rows()>0){
  foreach($cekaddendumlebihbesar->result() as $ceaddlb){
      if( (int)$ceaddlb->No_Urut_Add = (int)$dtdefault->No_Urut_Add ){
          $counting = (int)$counting+1;
      }else{
          $counting = (int)$counting+0;
      }
  }
  $counting = $counting;
}else{
...
$readonly_nil = ($counting<=0)?'readonly':'';
?>

  <form action="<?=base_url('ajax/addendumupdate/'.$this->uri->segment(3)."/".$dtdefault->Id_AddKontrak)?>" method="POST" enctype="multipart/form-data">
<input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash()?>" />    
  ...
  <div class="form-group col-sm-12">
    <div class="col-sm-12">
      Addendum<br/>
      <textarea class="form-control" name="Desc_KontrakResmi" id="Desc_KontrakResmi" placeholder="Desc Addendum"  required><?=$dtdefault->Desc_AddKontrak?></textarea>
    </div>
  </div>
  ...
  <div class="form-group col-sm-12">
    <div class="col-sm-6" style="color:#00008B">
      Nilai Rp<br/>
      <input type="text" class="form-control" name="Nilai_Rp" id="Nilai_Rp" placeholder="Nilai Rp" value="<?=str_replace(",",".",number_format($dtdefault->Nilai_Rp))?>"  onkeyup="splitInDots(this)" <?=$readonly_nil?> >
    </div>
  </div>
      ...
      <?php
            ...
            ?>
                <div style="max-height:150px;overflow:auto">
                    ...
                    ?>
                                <button type="button" class="btn btn-xs btn-danger btn-delete-file-lampiran" data="<?=base_url('ajax/delete_file_addendum/'.$dtdefault->Id_AddKontrak.'/'.$val)?>"><i class="fa fa-trash"></i></button> <a href="<?=base_url('uploads/fileapps/addendum/'.$val)?>" download> <?=$val?></a> <input type="hidden" name="filesebelumnya[]" value="<?=$val?>" >  <?=gettimefile($val)?><br/> <br/>
                    <?php
                                }
                            }
            ...
            ?>
                        <button type="button" class="btn btn-xs btn-danger btn-delete-file-lampiran" data="<?=base_url('ajax/delete_file_addendum/'.$dtdefault->Id_AddKontrak.'/'.$dtdefault->File_Lampiran)?>"><i class="fa fa-trash"></i></button>  <a href="<?=base_url('uploads/fileapps/addendum/'.$dtdefault->File_Lampiran)?>" download> <?=$dtdefault->File_Lampiran?></a> <?=gettimefile($dtdefault->File_Lampiran)?>
