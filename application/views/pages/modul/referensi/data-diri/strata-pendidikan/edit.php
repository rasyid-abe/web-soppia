...
<section class="content">
  ...
  <div class="box" style="border-top:0px solid">
    ...
    <div class="box-body">
      <form action="<?=base_url($this->uri->segment(1).'/'.$this->uri->segment(2)."/update/".$dtdefault->Kd_StrataPendidikan)?>" method="POST">
        <input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash()?>" />        
        <div class="form-group col-sm-12 <?=(!empty(form_error('name')))? 'has-error' :'';?>">
          <div class="col-sm-12">
            Nama Strata Pendidikan<br/>
            <input type="text" class="form-control" name="name" id="name" placeholder="Name"  value="<?=$dtdefault->Desc_StrataPendidikan?>" >
          </div>
        </div>  
